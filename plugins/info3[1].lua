local function callback_reply(extra, success, result)
	--icon & rank ------------------------------------------------------------------------------------------------
	userrank = "Member"
	if tonumber(result.from.peer_id) == 205305442
then
		userrank = "Master ⭐⭐⭐⭐"
		send_document(org_channel_id,"./plugins/sudo.webp", ok_cb, false)
	...
	--custom rank ------------------------------------------------------------------------------------------------
	local file = io.open("./data/info/"..result.from.id..".txt", "r")
	if file ~= nil then
...
local function callback_res(extra, success, result)
	...
	--custom rank ------------------------------------------------------------------------------------------------
	local file = io.open("./date/info/"..result.id..".txt", "r")
	if file ~= nil then
		usertype = file:read("*all")
	else
		usertype = "-----"
...
local function callback_info(extra, success, result)
	if success == 0 then
		return send_large_msg(org_channel_id, "آی دی وارد شده اشتباه است")
	end
	--icon & rank ------------------------------------------------------------------------------------------------
	if tonumber(result.id) == 139693972 then
		userrank = "Master ⭐⭐⭐⭐"
...
local function run(msg, matches)
	...
	elseif #matches == 2 then
		...
		--custom rank ------------------------------------------------------------------------------------------------
		local file = io.open("./data/info/"..msg.from.id..".txt", "r")
		if file ~= nil then
			usertype = file:read("*all")
		else
			usertype = "-----"
		...
		if not msg.reply_id then
			...
			if tonumber(msg.from.id) == 139693972 then
				userrank = "Master ⭐⭐⭐⭐"
				send_document("channel#id"..msg.to.id,"./plugins/sudo.webp", ok_cb, false)
			elseif is_sudo(msg) then
				userrank = "Sudo ⭐⭐⭐⭐⭐"
				send_document("channel#id"..msg.to.id,"./plugins/admin.webp", ok_cb, false)
			elseif is_admin1(msg) then
				userrank = "Admin ⭐⭐⭐"
				send_document("channel#id"..msg.to.id,"./icons/3.webp", ok_cb, false)
			elseif tonumber(msg.from.id) == tonumber(gp_leader) then
				userrank = "Leader ⭐⭐"
				send_document("channel#id"..msg.to.id,"./plugins/mod.webp", ok_cb, false)
			elseif is_momod(msg) then
				userrank = "Moderator ⭐"
				send_document("chat#id"..msg.to.id,"./plugins/mmbr.webp", ok_cb, false)
			else
				userrank = "Member"
			end
			--number ------------------------------------------------------------------------------------------------
